...
function setupAuth() {
  els.loginForm?.addEventListener("submit", async (event) => {
    ...
    if (error) alert("Ошибка авторизации: проверьте данные.");
  ...
  // onAuthStateChange сразу после подписки присылает событие INITIAL_SESSION
  // с текущей сессией (если она есть), поэтому отдельный вызов getSession()
  // не нужен — он раньше приводил к двойной загрузке данных при открытии страницы.
...
function renderDashboard() {
  ...
  if (els.deadlineList) els.deadlineList.innerHTML = upcoming.length ? upcoming.map((l) => `<div class="deadline-item"><strong>${escapeHtml(l.lotNumber)}</strong><span>${formatDate(l.deadline)}</span></div>`).join("") : `<div class="empty">Нет ближайших дедлайнов</div>`;
}
...
function openCalcDialog(id = "") {
  ...
  els.calcForm.elements.lotId.value = calc?.lotId || els.calcForm.elements.lotId.options[0]?.value || "";
...
async function importBackup(event) {
  ...
  reader.onload = async () => {
    try {
      ...
      if (!Array.isArray(imported?.lots) || !Array.isArray(imported?.calculations)) {
        alert("Файл бэкапа повреждён: ожидались поля lots и calculations.");
        return;
      }
      ...
      if (dbLots.length > 0) {
        const { error } = await supabaseClient.from("lots").upsert(dbLots);
        if (error) throw error;
      }
      if (dbCalcs.length > 0) {
        const { error } = await supabaseClient.from("calculations").upsert(dbCalcs);
        if (error) throw error;
      }
      await loadStateFromSupabase();
